...
# --- Constants for Scoring Scale ---
SCORING_SCALE = (
    "## SCORING SCALE (1-10)\n"
    "- **1-2:** Very Poor / Criterion is completely missed.\n"
    "- **3-4:** Poor / Significant issues present.\n"
    "- **5-6:** Fair / Meets minimum requirements but is flawed.\n"
    "- **7-8:** Good / Criterion is met well with minor room for improvement.\n"
    "- **9-10:** Excellent / Criterion is fully and expertly met."
)

WORKFLOW_SCORING_SCALE = (
    "## SCORING SCALE (1-10)\n"
...
def format_criteria(criteria: List[Dict[str, Any]]) -> str:
    """Format evaluation criteria for inclusion in the prompt."""
    return "\n".join(
        f"Criterion: {c['name']} (Weight: {c['weight']})"
        for c in criteria
    )

def format_criterion_blocks(criteria: List[Dict[str, Any]]) -> str:
    """Format the criterion-by-criterion evaluation section."""
    blocks = []
...
- **Weight:** {c['weight']}
- **Score:** `<Score>/10`
- **Justification:** `<Your rationale for the score. {c.get('description', '')}>`
""".strip()
        )
    return "\n\n".join(blocks)

def get_evaluator_instructions(
    criteria: Optional[List[Dict[str, Any]]] = None
) -> str:
    """
    Returns instructions for the evaluator agent.
    Allows dynamic criteria; falls back to default if not provided.
    """
    default_criteria = [
        {"name": "Factual Accuracy", "weight": 0.4, "description": "Referencing specific examples from the text if possible."},
        {"name": "Depth of Analysis", "weight": 0.3, "description": "Does the text cover the topic superficially or in detail?"},
        {"name": "Tone and Style", "weight": 0.2, "description": "Was the tone appropriate for the intended audience and topic?"},
        {"name": "Formatting", "weight": 0.1, "description": "Was the text well-structured with proper headings, lists, or paragraphs?"},
    ]
    criteria = criteria or default_criteria

    return (
        "## ROLE\n"
        "You are a meticulous research analyst with decades of experience in copywriting, proofreading, research writing.\n\n"
        "## OBJECTIVE\n"
        "Your objective is to objectively assess a given text output against a predefined set of weighted criteria and present your findings in a formal Markdown report.\n\n"
        "## INPUTS\n"
        "1.  **`output_text`**: The text generated by another AI model that you need to evaluate.\n"
        "2.  **`evaluation_criteria`**: A list of criteria, each with a description and a weight indicating its importance.\n\n"
        "## INSTRUCTIONS\n"
        "1.  Carefully review the `output_text`.\n"
        "2.  For each criterion, evaluate the text, assign a score from 1 to 10, and write a concise justification.\n"
        "3.  Calculate the final `weighted_average_score` using the formula: Σ(score * weight) / Σ(weight).\n"
        "4.  Structure your entire response as a single Markdown document according to the format below. **Do not use JSON for the output.**\n\n"
        f"{SCORING_SCALE}\n\n"
        "---\n\n"
        "**Output Text to Evaluate:**\n"
        "**Evaluation Criteria (with weights):**\n"
        f"{format_criteria(criteria)}\n\n"
        "# Quality Control Report\n\n"
        "### Final Assessment\n\n"
        "**Weighted Average Score:** `<Calculated Weighted Score>/10`\n\n"
        "**Executive Summary:** `<A concise summary of the output's performance. Highlight key strengths and critical areas for improvement based on the weighted criteria. This should give a reader a quick and accurate understanding of the result without needing to read the full breakdown.>`\n\n"
        "---\n\n"
        "### Criterion-by-Criterion Evaluation\n\n"
        f"{format_criterion_blocks(criteria)}"
    )

def get_workflow_qc_instructions() -> str:
    """
    Returns instructions for the workflow quality control agent.
    """
    return (
        "## ROLE\n"
